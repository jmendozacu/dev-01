...
//    $gifttemplate = Mage::getModel('mpblog/category')->load($id, 'category_id');
    ...
    try {
      ...
      if (isset($_FILES['document_promotion']['name']) && $_FILES['document_promotion']['name'] != '') {
        ...
        try {
          ...
          $uploader->setAllowRenameFiles(false);
          $uploader->setFilesDispersion(false);
          $path = Mage::getBaseDir('media').DS . 'magebuzz'. DS . 'doccument'. DS . 'promotion';
      ...
      }elseif($requestPost['document_promotion']){
        if(!isset($requestPost['document_promotion']['delete']) || $requestPost['document_promotion']['delete'] != 1){
          $requestPost['document_promotion'] = $requestPost['document_promotion']['value'];
        }else{
          $requestPost['document_promotion'] = NULL;
        }
      }
  ...
  public function massUpdateAttributeAction()
  {
    ...
    if ($posts && is_array($posts)) {
      ...
      Mage::register(
        Magpleasure_Blog_Block_Adminhtml_Attribute_Edit_Tabs_Posts::REGISTRY_POSTS,
        $posts,
        true
      );
      ...
      try {
        ...
        if ($result['skip']){
          ...
          $this->_getSession()->addNotice(
            $this->_helper()->__(
              "%s posts weren't changed. There are no changes required.",
              $result['skip']
            )
          );
        }
        ...
        if ($result['success']){
          ...
          $this->_getSession()->addSuccess(
            $this->_helper()->__(
              "%s posts were successfully updated",
              $result['success']
            )
          );
        }
