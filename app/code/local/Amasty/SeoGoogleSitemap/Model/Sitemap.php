...
class Amasty_SeoGoogleSitemap_Model_Sitemap extends Mage_Core_Model_Abstract
{
        ...
        do {
            ...
            foreach ($collection as $item) {
                ...
                if ($this->getProductsThumbs()) {
					if ($thumb = $item->getThumbnail()) {
						$thumb = Mage::getModel('catalog/product_media_config')->getMediaUrl($item->getThumbnail());
					}
                    if ($thumb) {
                        $xmlLine .= '<image:image>';
	...
	protected function _generateCms()
	{
        ...
        foreach ($collection as $item) {
			...
			$xmlLine   = '<url><loc>%s</loc><priority>%.2f</priority><changefreq>%s</changefreq>';
			$xmlParams = array(
				htmlspecialchars($this->_baseUrl . $item->getIdentifier()),
				$priority,
				$changefreq
			);
	...
	protected function _generateTags()
	{
		...
		foreach ($tags as $tag) {
			$this->_xml[] = sprintf('<url><loc>%s</loc><changefreq>%s</changefreq><priority>%.2f</priority></url>',
				htmlspecialchars($this->_baseUrl . 'tag/product/list/tagId/' . $tag->getTagId()),
				$changefreq,
				$priority
			);
		}
	...
	protected function _generateLanding()
	{
		...
		foreach ($landingPages as $page) {
            ...
            if (isset($title)) {
                $this->_xml[] = sprintf('<url><loc>%s</loc><changefreq>%s</changefreq><priority>%.2f</priority></url>',
                    htmlspecialchars($this->_baseUrl . $page->getUrl()),
                    $changefreq,
                    $priority
                );
            }
	...
	protected function _generateBrands()
	{
		...
		foreach ($brandPages->getSource()->getAllOptions() as $page) {
			if ($page['value']) {
				$this->_xml[] = sprintf('<url><loc>%s</loc><changefreq>%s</changefreq><priority>%.2f</priority></url>',
					$helper->getOptionUrl($attrCode, $page['value']),
					$changefreq,
					$priority
				);
			}
