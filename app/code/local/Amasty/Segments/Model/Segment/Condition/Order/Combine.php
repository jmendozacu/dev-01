...
//                '!()' => Mage::helper('amsegments')->__('is not one of'),
        ...
        public function index($websiteId, $conditonsCount, $combineCondition = null){
...
//            exit(1);
        ...
        protected function _getCollection($websiteId){
            
            $collection = Mage::getModel("amsegments/customer")
                    ->getCollection()
                    ->filterByWebsite($websiteId)
                    ->addProductIndexData($this->getId(), $this->getRule()->getId());
            
            return $collection;
        }
        
        public function process($websiteId, $combineCondition = null){
            
            $adapter = $this->getResource()->getReadConnection();
            
            $conditonsCount = 0;
            foreach ($this->getConditions() as $condition) {
                $condition->index($websiteId, $this);
                $conditonsCount++;
            }
            
            $this->index($websiteId, $conditonsCount, $combineCondition);
            
            $collection = $this->_getCollection($websiteId);
