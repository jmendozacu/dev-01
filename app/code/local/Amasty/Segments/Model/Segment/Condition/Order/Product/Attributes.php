...
//        }
        ...
        function getAttributeSelect($websiteId){
            $attributeSelect = $this->_getAttributeSelect($websiteId);
            return $attributeSelect;
        }
        
        protected function _getAttributeSelect($websiteId){
            ...
            if ($attribute->getAttributeCode() == 'category_ids') {
                ...
                $select->join(
                    ...
                    array())
                    ->where('store.website_id IN(?)', array(0, $websiteId));
                $condition = $this->getResource()->createConditionSql(
        ...
        public function index($websiteId, $combineCondition){
            $adapter = $this->getResource()->getReadConnection();
            
            $attributeSelect = $this->_getAttributeSelect($websiteId);
            
            $collection = Mage::getModel("amsegments/order")->getCollection()
                    ->addOrderItemData($websiteId);
