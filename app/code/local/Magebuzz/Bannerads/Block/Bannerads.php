...
class Magebuzz_Bannerads_Block_Bannerads extends Mage_Core_Block_Template {
  ...
  protected function _toHtml() {
    ...
    $collection = Mage::getModel('bannerads/bannerads')->getCollection()->addFieldToFilter('status', 1)->addFieldToFilter('block_position', array('in' => array($this->getBlockPosition()),))->addFieldToFilter('from_date', array('or' => array(0 => array('date' => TRUE, 'to' => $todayEndOfDayDate), 1 => array('is' => new Zend_Db_Expr('null')))), 'left')->addFieldToFilter('to_date', array('or' => array(0 => array('date' => TRUE, 'from' => $todayStartOfDayDate), 1 => array('is' => new Zend_Db_Expr('null')))), 'left')->setOrder('sort_order', "ASC");
    ...
    $collection->addFieldToFilter('store_id', array('in' => array(0, $storeId)))->getSelect()->group('t2.block_id');
		...
		foreach ($collection as $item) {
			...
			if ($currentCategoryId) {
				$filterCategoryIds = explode(',', $item->getCategory());
				...
				else if ($item->getCategoryType() == 3) {
