...
class Magebuzz_Bannerads_Block_Adminhtml_Bannerads_Edit_Tab_Main extends Mage_Adminhtml_Block_Widget_Form {
  protected function _prepareForm() {
    ...
    $fieldset->addField('block_max_width', 'text', array('label' => Mage::helper('bannerads')->__('Block max width'), 'name' => 'block_max_width',));
		...
		$categoryTypeField = $fieldset->addField('category_type', 'select', 
			array(
				'label' => Mage::helper('bannerads')->__('Category Type'), 
				'name' => 'category_type', 
				'values' => array(
					'1' => Mage::helper('bannerads')->__('All Categories'),
					'2' => Mage::helper('bannerads')->__('All categories except below'),
					'3' => Mage::helper('bannerads')->__('Certain Categories')
				), 
				'onchange' => "showCategory();"
		));
		
		$categoryTypeField->setAfterElementHtml('<script>
//<![CDATA
    ...
    var categoryTypeId = document.getElementById("bannerads_category_type");
    if($("bannerads_category_type").getValue() == 1){
...
//]]>
    ...
    $fieldset->addField('category', 'text', 
			array(
				'label' => Mage::helper('bannerads')->__('Category'), 
				'name' => 'category', 
				'container_id' => 'category_row_id', 
				'after_element_html' => 'separated by comma'
		));
