...
class MarginFrame_Dataflow_Model_Import_Adapter_Type_Product_Data_Category
    extends MarginFrame_Dataflow_Model_Import_Adapter_Type_Product_Data_Abstract
{
    ...
    protected function _initializeCategoryTree()
    {
        ...
        if (is_null($rootCategory->getId())) {
            throw new MarginFrame_Dataflow_Model_Import_Adapter_Exception('Root category not found.');
        }
    ...
    public function processData(array &$data)
    {
        try {
            if (isset($data['categories']) && $data['categories'] || isset($data['category_ids']) && $data['category_ids']) {
                ...
                //IT-1859
                if(Mage::getStoreConfig('ttfpim/import/catalog_replace')){
                    //removed old catalog
                ...
                if (!empty($categoryIds)) {
                	...
                	foreach ($categoryIds as $categoryId) {
                		$logerror = $data['product_id'].','.$categoryId;
                		try {
	                    	$this->_writeConnection->insertOnDuplicate(
	                            $this->_categoryProductTable,
	                            array('product_id' => $data['product_id'], 'category_id' => $categoryId, 'position' => 10001),
	                        	array('position' => new Zend_Db_Expr('position'))
	                        );
                        } catch (Exception $e) {
                        	//Mage::log($logerror, null, 'mgf_import.log');
                        }
                    }
                }
            }
        } catch (Exception $e) {
            throw new MarginFrame_Dataflow_Exception("Can't save category relation.");
        }
