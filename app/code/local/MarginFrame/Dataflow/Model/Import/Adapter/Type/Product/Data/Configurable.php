...
class MarginFrame_Dataflow_Model_Import_Adapter_Type_Product_Data_Configurable
    extends MarginFrame_Dataflow_Model_Import_Adapter_Type_Product_Data_Abstract
{
    ...
    public function processData(array &$data)
    {
        if (!empty($data['associated'])) {
            $this->_assignedProductData[$data['sku']] = explode(',', $data['associated']);
        }
        if (!empty($data['config_attributes']) &&  $data['type_id'] == Mage_Catalog_Model_Product_Type_Configurable::TYPE_CODE) {
            $this->_configAttributes[$data['sku']] = explode(',', $data['config_attributes']);
        }
        if (!empty($data['config_pricing'])) {
            $this->_configAttributePrices[$data['sku']] = explode('|', $data['config_pricing']);
        }
    ...
    public function afterProcess($skuToId)
    {
        if (!empty($this->_configAttributes)) {
            ...
            foreach ($this->_configAttributes as $sku => $configAttributes) {
            	//ipune skip sku not exist
            	if(!array_key_exists($sku, $skuToId) || $skuToId[$sku] == null){
            		//Mage::log('sku='.$sku.' id='.$skuToId[$sku], null, 'mgf_import.log');
            		continue;
            	}
                ...
                foreach ($configAttributes as $position => $configAttribute) {
                    if (!isset($this->_availableConfigAttributes[$configAttribute])) {
                        throw new MarginFrame_Dataflow_Model_Import_Adapter_Exception(sprintf('Attribute %s is not configurable', $configAttribute));
                    }
        ...
        if (!empty($this->_assignedProductData)) {
            foreach ($this->_assignedProductData as $sku => $assignedProducts) {
                ...
                foreach ($assignedProducts as $assignedProduct) {
                	//ipune skip sku not exist
                	if(!array_key_exists($assignedProduct, $skuToId) || $skuToId[$assignedProduct] == null){
                		//Mage::log('sku='.$sku.' id='.$skuToId[$assignedProduct], null, 'mgf_import.log');
                		continue;
                	}
                	$values = array(
