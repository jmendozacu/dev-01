...
$scheduleTable = $this->getTable('marginframe_dataflow/profile_schedule');
...
$sql = "CREATE TABLE IF NOT EXISTS `{$profileTable}` (
  ...
  `can_create_urlkey` TINYINT(1) NOT NULL,
...
$sql = "CREATE TABLE IF NOT EXISTS `{$scheduleTable}` (
  ...
  KEY `IDX_MGF_CRON_SCHEDULE_PROFILE_ID` (`schedule_id`),
  KEY `IDX_MGF_CRON_SCHEDULE_SCHEDULED_AT_STATUS` (`scheduled_at`,`status`),
  KEY `MGF_DATAFLOW_PROFILE_SCHEDULE_ID` (`profile_id`),
  CONSTRAINT `MGF_DATAFLOW_PROFILE_SCHEDULE_ID` FOREIGN KEY (`profile_id`) REFERENCES `{$profileTable}` (`entity_id`) ON DELETE CASCADE
