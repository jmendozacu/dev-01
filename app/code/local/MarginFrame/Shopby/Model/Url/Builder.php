...
class MarginFrame_Shopby_Model_Url_Builder
{
    /** @var  string */
    public $moduleName;
    ...
    protected function getParamPart()
    {
        ...
        if ($this->mode && !$this->isSomeSearch()) {
            ...
            foreach ($this->effectiveQuery as $origAttrCode => $ids)
            {
                ...
                if (isset($options[$attrCode])){ // it is filterable attribute
                    if ($this->mode == MarginFrame_Shopby_Model_Source_Url_Mode::MODE_SHORT) {
        ...
        $glue = ($this->mode == MarginFrame_Shopby_Model_Source_Url_Mode::MODE_SHORT) ? Mage::getStoreConfig('amshopby/seo/option_char') : '/';
    ...
    protected function getBasePart($paramPart)
    {
        ...
        elseif ($this->getCategoryId() == $rootId) {
            ...
            switch ($this->mode) {
                case MarginFrame_Shopby_Model_Source_Url_Mode::MODE_DISABLED:
                    $needUrlKey = true;
                    break;
                case MarginFrame_Shopby_Model_Source_Url_Mode::MODE_MULTILEVEL:
                    $needUrlKey = !$this->isBrandPage();
                    break;
                case MarginFrame_Shopby_Model_Source_Url_Mode::MODE_SHORT:
    ...
    {
        return $this->mode == MarginFrame_Shopby_Model_Source_Url_Mode::MODE_MULTILEVEL || $this->mode == MarginFrame_Shopby_Model_Source_Url_Mode::MODE_DISABLED;
    }
}
