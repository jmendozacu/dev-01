...
class MarginFrame_Shopby_Helper_Layer_View_Strategy_Attribute extends MarginFrame_Shopby_Helper_Layer_View_Strategy_Modeled
{
    ...
    {
        $template = 'marginframe/amshopby/attribute.phtml';

        $isSwatchesDisplayType = is_object($this->model) && $this->model->getDisplayType() == MarginFrame_Shopby_Model_Source_Attribute::DT_MAGENTO_SWATCHES;
        if ($isSwatchesDisplayType) {
            if ($this->isSwatchesAvailable()) {
                $template = 'configurableswatches/catalog/layer/filter/swatches.phtml';
            } else {
                $this->model->setDisplayType(MarginFrame_Shopby_Model_Source_Attribute::DT_LABELS_ONLY);
            }
        }
    ...
    protected function prepareItems()
    {
        ...
        foreach ($items as $item){
            /** @var MarginFrame_Shopby_Model_Catalog_Layer_Filter_Item $item */
    ...
    public function getIsExcluded()
    {
        ...
        $hideByConfigurableSwatches = $this->model->getDisplayType() == MarginFrame_Shopby_Model_Source_Attribute::DT_MAGENTO_SWATCHES && $this->isSwatchesAvailable();
