 ...
 */
class MarginFrame_Shopby_Adminhtml_FilterController extends Mage_Adminhtml_Controller_Action
{
    // show grid
    ...
    protected function _checkOldTemplates()
    {
        ...
        foreach (Mage::app()->getStores() as $store){
            ...
            if (is_dir($excessPath)){
                $msg = $this->__('In case you need to modify the module templates please copy files from app/design/frontend/base/default/template/marginframe/amshopby/  to your custom theme  app/design/frontend/PACKAGE/THEME/template/marginframe/amshopby/');
                $this->_getSession()->addNotice($msg);
                break;
            }
        }
    ...
    protected function _checkConflicts()
    {
        ...
        foreach ($classes as $type => $names){
            foreach ($names as $name){
                ...
                if (substr($name, 0, 6) != 'MarginFrame'){
                    $msg = $this->__('There is a conflict(s) with some other extension: class %s. If the module works incorrect, consider our <a href="http://marginframe.com/installation-service.html">Installation Service</a>.', $name);
                    $this->_getSession()->addNotice($msg);
                    break(2);
                }
            }
    ...
    {
        /** @var MarginFrame_Shopby_Helper_Migration $migrations */
        ...
        if ($next) {
            ...
            $msg = $this->__('Migration issue detected; <a href="'. $link .'">click here</a> to fix. Please contact MarginFrame Support in case issue not fixed.');
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            if ($model->getData('display_type') == MarginFrame_Shopby_Model_Catalog_Layer_Filter_Price::DT_FROMTO) {
