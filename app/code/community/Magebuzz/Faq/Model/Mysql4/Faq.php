...
class Magebuzz_Faq_Model_Mysql4_Faq extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object)
    {
        ...
        if ($newCategoryIds) {
            ...
            $tableStore = $this->getTable('faq/faq_store');
            $insertStore = array_diff($newStores, $oldStores);
            $deleteStore = array_diff($oldStores, $newStores);

            if ($deleteStore) {
                $whereStore = array(
                    'faq_id = ?' => (int)$object->getId(),
                    'store_id IN (?)' => $deleteStore
                );

                $this->_getWriteAdapter()->delete($tableStore, $whereStore);
            }

            if ($insertStore) {
                $dataStore = array();

                foreach ($insertStore as $storeId) {
                    $dataStore[] = array(
                        'faq_id' => (int)$object->getId(),
                ...
                $this->_getWriteAdapter()->insertMultiple($tableStore, $dataStore);
            }
        return parent::_afterSave($object);
        }
    }
